...
static void getMedianMaxRootNumbers(cv::Mat seg, double &medrtno, double &maxrtno)
{
    cv::Mat seg1, seg2, diff, m, m2;
    double *mptr;
    int zidx = 0;

    hconcat(cv::Mat::zeros(seg.rows, 1, seg.type()), seg, seg1);
    hconcat(seg, cv::Mat::zeros(seg.rows, 1, seg.type()), seg2);
    diff = seg1 - seg2;
...
static double getperimeterlength(vector<vector<cv::Point>> &contours)
{
...
static void getavgrootradius(ListofListsRef<double> radii, double &medradius, double &avgradius)
{
    int rsize = 0;
    avgradius = 0.0;
    medradius = 0.0;
...
static void getvolumesurfacearea(cv::Mat skeleton, cv::Mat dist, ListofListsRef<cv::Point> segments, PointList overlap, double &volume, double &surfacearea)
{
...
static void getdiameterrangehistograms_new(cv::Mat skeleton, cv::Mat dist, ListofListsRef<cv::Point> segments, PointList overlap, Values rranges,
...
/*double& coarse, double& medium, double& fine*/ // double dlo, double dhi)
{
    ...
    cv::Mat radrange = cv::Mat::zeros(dist.size(), CV_8UC1);
...
static void getdiameterrangehistograms(cv::Mat skeleton, cv::Mat dist, ListofListsRef<cv::Point> segments, PointList overlap, Values rranges,
                                       ValueList rootlengthhist, ValueList projareahist, ValueList sahist, ValueList vhist, double &rootlength, double &volume, double &sarea)
/*double& coarse, double& medium, double& fine*/ // double dlo, double dhi)
{
    int i, j, k, x, y, xinc, yinc, currx, curry, nextx, nexty, nnextpt, fxinc, fyinc, didx;
...
static void getdiameterrangehistograms(cv::Mat skeleton, cv::Mat dist, ListofListsRef<cv::Point> segments, PointList overlap, Values rranges,
                                ValueList rootlengthhist, ValueList projareahist, ValueList sahist, ValueList vhist, double &rootlength, double &volume, double &sarea, cv::Mat &rtdpoints)
/*double& coarse, double& medium, double& fine*/ // double dlo, double dhi)
{
    int i, j, k, x, y, xinc, yinc, currx, curry, nextx, nexty, nnextpt, fxinc, fyinc, didx;
    double rad = 0, sqr = sqrt(2.0), dv = 0;
    ...
    rtdpoints = cv::Mat::zeros(static_cast<int>(rdpoints.size()), 2, CV_64FC1);
...
static void getshallowdeeproots(ListofListsRef<cv::Point> segments, double &shallow, double &medium, double &deep, double &avgangle)
{
    shallow = 0.0;
    medium = 0.0;
    deep = 0.0;
    ...
    for (i = 0, szcount = 0; i < csz; i++)
    {
        ...
        for (j = 0; j < nsamples; j++)
        {
            ...
            if (npoints > 1)
            {
                cv::calcCovarMatrix(m2.rowRange(0, npoints), cov, me, cv::COVAR_ROWS | cv::COVAR_NORMAL);
                cv::eigen(cov, lam, ev);
                evptr = ev.ptr<double>();
...
static void getshallowdeeprootsth(ListofListsRef<cv::Point> segments, double &shallow, double &medium, double &deep, double &avgangle)
{
    shallow = 0.0;
    medium = 0.0;
    deep = 0.0;
    ...
    int nthreads = cv::getNumberOfCPUs() - 3;
    ...
    vector<cv::Mat> m(nthreads), m2(nthreads);
        ...
        t[th] = thread([&](int thr)
            ...
            for (i = 0; i < csz; i++)
            {
                ...
                for (j = 0; j < nsamples; j++)
                {
                    ...
                    if (npoints > 1)
                    {
                        cv::calcCovarMatrix(m2[thr].rowRange(0, npoints), cov, me, cv::COVAR_ROWS);
                        cv::eigen(cov, lam, ev);
                        evptr = ev.ptr<double>();
                        angle = abs((atan(evptr[1] / evptr[0]) * 180.0) / CV_PI);
                        tavgangle[thr] += angle;
...
static bool checkProgress(feature_config *config, QString str)
{
    if (!config->batchmode)
    {
...
{
    cv::Mat img, out, seg, lsim, lsimcropped, skel, dilatedskel, dist;
    cv::Mat perimeter, hull, fhull, chull, dilatedhull, selement, merged, holes, pmap, nearest;
    vector<vector<cv::Point>> contours;
    vector<vector<int>> holepts;
    vector<cv::Mat> channels, hch;
    vector<vector<cv::Point>> hullpt;
    vector<cv::Vec4i> hierarchy;
    vector<int> centroids, colorcodes;
    Points branchpts, endpts, overlappts;
    ListofLists<cv::Point> rootsegments;
    ...
    int nfeatures;
    cv::Mat rtdpoints;
    cv::RotateFlags flag = cv::ROTATE_90_CLOCKWISE, revflag = cv::ROTATE_90_COUNTERCLOCKWISE;
    Points simplified;
    ...
    case 90:
        ...
        revflag = cv::ROTATE_90_COUNTERCLOCKWISE;
        break;
    case 180:
        flag = cv::ROTATE_180;
        revflag = cv::ROTATE_180;
        break;
    case 270:
        flag = cv::ROTATE_90_COUNTERCLOCKWISE;
        revflag = cv::ROTATE_90_CLOCKWISE;
        break;
    }
    ...
    if (config->rotation != 0)
        cv::rotate(config->input, img, flag);
    else
        img = config->input.clone();
    
    if (checkProgress(config, "Thresholding the input image ..."))
        return;
    ...
    if (img.empty())
    {
        cv::Mat m1 = cv::Mat::zeros(out.size(), out.type());
        cv::Mat m2 = cv::Mat::ones(out.size(), out.type()) * 255u;
    ...
    if (checkProgress(config, "Filtering noise ..."))
        return;
    ...
    else
    {
        ...
        else
        {
            ...
            if (config->filterfgnoise)
                seg = 255 - getImageFromComponents(out.size(), getConnectedComponents(255 - out, (config->maxcompsizefg / pixel2mm) / pixel2mm));
        }
    }
    ...
    if (checkProgress(config, "Simplifying contours ..."))
        return;
    ...
    if (checkProgress(config, "Determining distance map ..."))
        return;

    dist = bwdist(lsim);
    
    if (checkProgress(config, "Skeletonizing ..."))
        return;

    // std::ofstream ofs;
    // ofs.open("bwdist.csv");
    // ofs << format(dist, cv::Formatter::FMT_CSV) << std::endl;
    // ofs.close();

    skel = bwskel(lsim, dist);
    // skel = bwthin(lsim);

    if (checkProgress(config, "Finding the contours ..."))
        return;
    ...
    {
        if (checkProgress(config, "Finding the convex hull ..."))
            return;
        ...
        if (contours.size() > 0)
        {
            if (config->keepLargest)
                cv::convexHull(contours[0], hullpt[0], false);
            else
            {
                ...
                cv::convexHull(cont, hullpt[0], false);
            }
        }
    }

    if (checkProgress(config, "Finding the root topology ..."))
        return;

    getroottopology(skel, dist, branchpts, endpts, overlappts, rootsegments, config->enableRootPruning, config->rootPruningThreshold);
    
    if (checkProgress(config, "Extracting the features ..."))
        return;
    ...
    t1 = new thread([&]()
    {
        ...
        nwarea = double(cv::countNonZero(lsim));
    ...
    getdiameterrangehistograms_new(skel, dist, rootsegments, overlappts, radiusranges, rootlengthhist, projareahist, sahist, vhist, rootlength, volume, surfacearea);
    // if (config->batchmode)
    ...
    if (config->roottype == 0)
    {
        auto fresult = cvutil::find(lsim > 0, FindType::Subscripts);
    ...
    if (maxradius > 0 && lsim.rows > 1 && config->roottype == 0)
        lowerrootarea = double(cv::countNonZero(lsim.rowRange(maxradrowidx, lsim.rows)));
    else
        lowerrootarea = NAN;
    ...
    if ((!config->batchmode) || (config->batchmode && config->savesegmented))
    {
        ...
        // if (config->invertimage)
        //{
        m = cv::Mat::zeros(seg.size(), seg.type());
        ...
        if (config->rotation != 0)
        {
            cv::rotate(m, config->seg, revflag);
        }
        else
        {
            m.copyTo(config->seg);
    ...
    if (checkProgress(config, "Generating output image ..."))
        return;
    ...
    else
    {
        ...
        m = cv::Mat::zeros(seg.size(), lsim.type());
        lsim.copyTo(m.rowRange(rminseg, rmaxseg).colRange(cminseg, cmaxseg));
        lsim = m.clone();
        m = cv::Mat::zeros(seg.size(), CV_32FC1);
    ...
    if (config->roottype == 0)
    {
        cv::Mat m = cv::Mat::zeros(seg.size(), lsim.type());

        if (config->showConvexHull)
        {
            hull = cv::Mat::zeros(rmaxseg - rminseg, cmaxseg - cminseg, CV_8UC1);
            cv::drawContours(hull, hullpt, -1, cv::Scalar(255), 2, 8, cv::noArray(), 0, cv::Point());
        ...
        if (config->showConvexHull)
            cv::dilate(hull, dilatedhull, cv::getStructuringElement(cv::MORPH_ELLIPSE, cv::Size(3, 3)));
    }
    ...
    {
        perimeter = cv::Mat::zeros(rmaxseg - rminseg, cmaxseg - cminseg, CV_8UC1);
        cv::drawContours(perimeter, contours, -1, cv::Scalar(255), config->contourwidth, 8, hierarchy);
    ...
    if (config->roottype == 0)
    {
        ...
        {
            // skel = skel + perimeter;
            selement = cv::getStructuringElement(cv::MORPH_ELLIPSE, cv::Size(config->medialaxiswidth, config->medialaxiswidth));
            cv::dilate(skel, dilatedskel, selement);
        ...
        {
            if (config->showDistMap)
                channels.push_back(lsim - dilatedskel + m);
            else
                channels.push_back(lsim - dilatedskel);

            channels.push_back((lsim - dilatedskel) + dilatedskel);
        }
        ...
        holes = cvutil::addBorder(255 - holes, 0);
        ...
        if (holepts.size() <= 1)
        {
            ...
            if (config->rotation != 0)
            {
                cv::rotate(merged, config->processed, revflag);
            }
            else
            {
        ...
        if (config->showHoles)
        {
            holepts = getholes(holepts, cv::Size(cmaxseg - cminseg + 2, rmaxseg - rminseg + 2));
            if (holepts.size() > 5)
            {
            ...
            merged = drawtopology_wholeroot(merged, cv::Scalar_<int>(rmaxseg, rminseg, cmaxseg, cminseg), holepts, colorcodes, colors);
        }
    }
    else
    {
        ...
        {
            skel = cv::Mat::zeros(lsim.size(), CV_8UC1);
            dilatedskel = cv::Mat::zeros(lsim.size(), CV_8UC1);
            branchpts.clear();
            endpts.clear();
            overlappts.clear();
        }
        ...
        if (config->showDistMap)
            ...
            dmap = cv::Mat::zeros(lsim.size(), CV_8UC1);

        if (config->showMedialAxis)
        {
            if (!config->showMedialAxisDiameter)
                merged = drawtopology_disconnectedroot(lsim, dmap, cv::Scalar_<int>(rmaxseg, rminseg, cmaxseg, cminseg), dilatedskel, branchpts, endpts, overlappts, simplified, config->medialaxiswidth);
            else
            {
                vector<cv::Mat> lch;
                ...
                cv::Mat dskel = drawmedialaxis(dist, skel, radiusranges, config->medialaxiswidth);
                merged = merged + dskel;
                // vector<Mat> dch;
                // split(dskel, dch);
